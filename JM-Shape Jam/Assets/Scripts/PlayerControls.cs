    ...
    void Update()
    {
        ...
        if ((Input.GetKey("space") || Input.GetKey("z")) && (Time.time > nextFire)) 
        {
            ...
            if(currentLevel >= 5)
            {
                ...
                Vector3 bottomOffset = new Vector3(0, -0.2f, 0);
                Instantiate(projectile, transform.position + topOffset, transform.rotation);
                Instantiate(projectile, transform.position + bottomOffset, transform.rotation);
            }
