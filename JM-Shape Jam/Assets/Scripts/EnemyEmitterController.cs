...
public class EnemyEmitterController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Time.time > nextSpawn)
        {
            ...
            |**** Add your code below ****|
            \*****************************/
            for(int i = 0; i < playerController.currentLevel; i++)//checks for the enemy so that it can spawn enemies for what level it is on
            {
                float randomX = Random.Range(-6.0f, 6.0f);//sets random position
                ...
                {
                    Instantiate(GiantEnemy, enemyPosition, transform.rotation);
                }
            }
